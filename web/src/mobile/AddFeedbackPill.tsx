...
export function AddFeedbackPill({
  ...
  composing: boolean;
  onAdd: (anchor: PendingAnchor, quote: string, rect: { left: number; top: number }) => void;
}) {
  ...
  useEffect(() => {
    ...
    // A pane scroll slides the code out from under the frozen rect — dismiss
    // rather than point the pill at the wrong line (the selection itself survives).
    const scope = scopeRef.current;
  ...
  // wrap; 72px ≈ half the pill plus a margin.
  const left = Math.min(Math.max(cap.left, 72), window.innerWidth - 72);
  return (
    <button
      type="button"
      onMouseDown={(e) => e.preventDefault()}
      onClick={() => {
        onAdd(cap.anchor, cap.quote, { left: cap.left, top: cap.top });
