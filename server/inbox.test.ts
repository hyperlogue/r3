...
describe("nudgeText", () => {
  ...
  test("leads with the review id, so two reviews never send identical text", () => {
    const a = nudgeText("review_aaa", "Same title", "submitted", t0);
    const b = nudgeText("review_bbb", "Same title", "submitted", t0);
    expect(a).not.toBe(b);
  ...
  test("two Submits on the SAME review differ, so neither can be deduped away", () => {
    const first = nudgeText("review_aaa", "T", "submitted", t0);
    const second = nudgeText("review_aaa", "T", "submitted", new Date(t0.getTime() + 1000));
    expect(first).not.toBe(second);
  });

  test("tells the agent what to run, but only when there is something to fetch", () => {
    expect(nudgeText("review_a", "T", "submitted", t0)).toContain("Run: r3 prompt review_a");
    expect(nudgeText("review_a", "T", "approved", t0)).not.toContain("Run:");
    expect(nudgeText("review_a", "T", "abandoned", t0)).not.toContain("Run:");
  });

  test("names the terminal outcome, since there is no exit code to carry it", () => {
    expect(nudgeText("review_a", "T", "approved", t0)).toContain("approved");
    expect(nudgeText("review_a", "T", "abandoned", t0)).toContain("abandoned");
  });
});
