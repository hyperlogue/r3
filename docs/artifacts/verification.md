...
| `test-preview-cache.ts` | Real HTTP-cache revalidation after Markdown source switches, page refresh, and old-version visits; HTML refresh/version reuse, repeated gate checks, opaque origins, and deletion cleanup. Accepts the same caller-installed Playwright engine settings as the compatibility suite |
