...
| `test-preview-cache.ts` | Real HTTP-cache revalidation for documents and trusted runtime scripts after Markdown source switches, page refresh, and old-version visits; visible-only startup in a seven-file stack, direct file jumps and retained frames after folding/scrolling, outer Markdown and inner HTML scroll restoration, repeated gate checks, opaque origins, and deletion cleanup. Accepts the same caller-installed Playwright engine settings as the compatibility suite |
